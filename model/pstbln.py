...
class ST_BLN_block(nn.Module):
    def __init__(self, in_channels, topology, blocksize, layer_ind, num_point, stride=1, residual=True):
        ...
        self.bln = BilinearMapping(in_channels, out_channels, num_point)
        self.tcn = TemporalConvolution(out_channels, out_channels, stride=stride)
        self.relu = nn.ReLU()
        if not residual:
            self.residual = lambda x: 0
        elif ((in_channels == out_channels) and (stride == 1)) or layer_ind == 0:
...
class PSTBLN(nn.Module):
    def __init__(self, topology, blocksize, num_class=60, num_point=25, num_person=2, in_channels=3):
        ...
        self.layers = nn.ModuleDict(
            {'l{}'.format(i): ST_BLN_block(self.in_channels, self.topology, self.block_size, i, num_point=num_point)
